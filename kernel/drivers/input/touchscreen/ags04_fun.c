...
if((gpio_get_value(gpio_sensor))==0)
{
	...
	if(readbuf==0)
	{
		if(key1_press==1)
		{
			input_event(input_dev, EV_KEY, KEY_HOME, 0);	
			input_sync(input_dev);
			key1_press=0;
		}
		if(key2_press==1)
		{
			input_event(input_dev, EV_KEY, KEY_MENU, 0);	
			input_sync(input_dev);
			key2_press=0;
		}
		if(key4_press==1)
		{
			input_event(input_dev, EV_KEY, KEY_BACK, 0);	
			input_sync(input_dev);
			key4_press=0;
		}
		if(key8_press==1)
		{
			input_event(input_dev, EV_KEY, KEY_MUTE, 0);	
			input_sync(input_dev);
			key8_press=0;
		}
	}
	else
	{
		if((key1_press==0)&&(key2_press==0)&&(key4_press==0)&&(key8_press==0))
		{
			...
			if(readbuf==0x01)
			{
				input_event(input_dev, EV_KEY, KEY_HOME, 1);	
				input_sync(input_dev);
				key1_press=1;
			}
			else if(readbuf==0x02)
			{
				...
				input_event(input_dev, EV_KEY, KEY_MENU, 1);	
				input_sync(input_dev);
				key2_press=1;
			}
			else if(readbuf==0x04)
			{
				input_event(input_dev, EV_KEY, KEY_BACK, 1);	
				input_sync(input_dev);
				key4_press=1;
			}
			else if(readbuf==0x08)
			{
				input_event(input_dev, EV_KEY, KEY_MUTE, 1);	
				input_sync(input_dev);
				key8_press=1;
			}
			else
...
printk("ags04_probe...aa\n");
	...
	ags04_ts->irq_pin = of_get_named_gpio_flags(np, "sensor-int", 0, &irq_flags);
...
for(i = 0; i < MAX_KEY_CNT; i++)
	...
	input_set_capability(input_dev, EV_KEY, ags04_key_table_tc[i].keyCode);

	input_dev->name = AGS04_NAME;
	input_dev->phys = "ags04/input2";
