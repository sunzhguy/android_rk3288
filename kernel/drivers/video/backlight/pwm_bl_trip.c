...
static int pwm_backlight_probe(struct platform_device *pdev)
{
	...
	int ret;
printk("pwm_backlight_probe...........start trip\n");
	if (!data) {
	...
	if (IS_ERR(pb->pwm)) {
		...
		pb->pwm = pwm_request(data->pwm_id, "pwm-backlight_trip");
		if (IS_ERR(pb->pwm)) {
...
printk("data->pwm_id=%d..........\n",data->pwm_id);
	...
	dev_set_name(&pdev->dev, "trip_rk28_bl");
#endif
	bl = backlight_device_register_trip(dev_name(&pdev->dev), &pdev->dev, pb,
...
static int pwm_backlight_remove(struct platform_device *pdev)
{
	...
	backlight_device_unregister_trip(bl);
	pwm_backlight_power_off(pb);
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:pwm-backlight_trip");
